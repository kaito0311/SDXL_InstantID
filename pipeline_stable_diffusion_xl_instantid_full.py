...
class LongPromptWeight(object):
    ...
    def get_weighted_text_embeddings_sdxl(
        ...
        for i in range(len(prompt_token_groups)):
            ...
            token_tensor = torch.tensor([prompt_token_groups[i]], dtype=torch.long)
            weight_tensor = torch.tensor(prompt_weight_groups[i], dtype=torch.float16)

            token_tensor_2 = torch.tensor([prompt_token_groups_2[i]], dtype=torch.long)

            # use first text encoder
            prompt_embeds_1 = pipe.text_encoder(token_tensor, output_hidden_states=True)
            prompt_embeds_1_hidden_states = prompt_embeds_1.hidden_states[-2]

            # use second text encoder
            prompt_embeds_2 = pipe.text_encoder_2(
                token_tensor_2, output_hidden_states=True
            )
            ...
            neg_token_tensor = torch.tensor(
                [neg_prompt_token_groups[i]], dtype=torch.long
            )
            neg_token_tensor_2 = torch.tensor(
                [neg_prompt_token_groups_2[i]], dtype=torch.long
            )
            neg_weight_tensor = torch.tensor(
                neg_prompt_weight_groups[i], dtype=torch.float16
            )

            # use first text encoder
            neg_prompt_embeds_1 = pipe.text_encoder(
                neg_token_tensor, output_hidden_states=True
            )
            neg_prompt_embeds_1_hidden_states = neg_prompt_embeds_1.hidden_states[-2]

            # use second text encoder
            neg_prompt_embeds_2 = pipe.text_encoder_2(
                neg_token_tensor_2, output_hidden_states=True
            )
...
class StableDiffusionXLInstantIDPipeline(StableDiffusionXLControlNetPipeline):
    ...
    def __init__(
        ...
        image_proj_model: Resampler = None,
    ):
        super().__init__(
            vae=vae,
    ...
    def set_image_proj_model(self, model_ckpt, image_emb_dim=512, num_tokens=16):
        ...
        self.register_modules(image_proj_model=image_proj_model)
    ...
    def enable_sequential_cpu_offload(
        ...
        is_pipeline_device_mapped = (
            self.hf_device_map is not None and len(self.hf_device_map) > 1
        )
